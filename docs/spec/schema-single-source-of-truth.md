...
|---|-----------|-------------|
| 1 | Released AAC schemas SHALL remain immutable published artifacts, and the current drift-corrected schema SHALL be identified as `0.17.0`. The AAC schema version SHALL be independent of the application version, RO-Crate specification version, RO-Crate profile version, and per-canvas `project.version`. Only the selected current schema is a fully supported runtime contract. | Inspect the versioned artifacts and current manifest selection; change the application version and verify that the declared AAC schema version does not change. |
| 2 | The `0.17.0` schema SHALL describe the complete current AAC model without v2 additions, including `Person.functionRoles`, `Person.localTitle`, `Project.creator`, `Project.license`, and milestone objects containing `description` and `kpi`. | Validate representative complete-canvas fixtures and compare every current AAC field against the schema; verify that proposed v2-only fields are absent. |
...
| 6 | Current-version persisted and exported data SHALL be validated at runtime against the `0.17.0` JSON Schema. Validation findings SHALL be available as structured diagnostics for both logging and user-visible display, without a separately maintained structural validator becoming authoritative. | Exercise valid and invalid `0.17.0` fixtures at every persistence and export boundary; assert schema-derived diagnostics and their logged and displayed forms. |
...
- Current `0.17.0` data rejects undeclared properties.
